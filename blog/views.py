...
def index(request):
    ...
    most_popular_posts = Post.objects.prefetch_related('author').annotate(likes_count=Count('likes')).order_by('-likes_count')[0:5]
...
def tag_filter(request, tag_title):
    most_popular_posts = Post.objects.prefetch_related('author').annotate(likes_count=Count('likes')).order_by(
        '-likes_count')[0:5]
    most_popular_posts_ids = [post.id for post in most_popular_posts]
    posts_with_comments = Post.objects.filter(id__in=most_popular_posts_ids).annotate(comments_count=Count("comments"))
    ids_and_comments = posts_with_comments.values_list("id", "comments_count")
